...
#define STEPS_PER_DEGREE        (STEPS_PER_REV * GEAR_RATIO / 360.0f)  /* Steps per joint degree / 每关节度的步数 */
#endif

#ifndef DEGREES_PER_STEP
#define DEGREES_PER_STEP        (360.0f / (STEPS_PER_REV * GEAR_RATIO)) /* Degrees per step / 每步度数 */
#endif
...
#define UNITS_PER_DEGREE       STEPS_PER_DEGREE    /* Motor units = steps; kinematics and engine share one scale / 电机单位=步数, 运动学与引擎统一量纲 */
#endif
...
#define HOMING_BACKOFF_M1_DEG   101.0f       /* M1 backoff angle (deg) / M1 回退角度 */
#endif

#ifndef HOMING_BACKOFF_M2_DEG
#define HOMING_BACKOFF_M2_DEG   24.0f        /* M2 backoff angle (deg) / M2 回退角度 */
#endif

#ifndef HOMING_BACKOFF_M3_DEG
#define HOMING_BACKOFF_M3_DEG   7.0f         /* M3 backoff angle (deg) / M3 回退角度 */
#endif
