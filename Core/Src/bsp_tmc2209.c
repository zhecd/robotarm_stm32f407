...
static uint8_t TMC2209_CalcCRC8(const uint8_t *datagram, uint8_t len)
{
    uint8_t crc = 0U;
    ...
    for (uint8_t i = 0U; i < len; i++) {
        ...
        for (uint8_t bit = 0U; bit < 8U; bit++) {
            if ((uint8_t)((crc >> 7) ^ (current_byte & 0x01U)) != 0U) {
...
static uint32_t TMC2209_ConvertMicrostepsToMres(uint16_t microsteps)
{
    switch (microsteps) {
    case 256U: return 0U;
    case 128U: return 1U;
...
static void TMC2209_WriteReg(UART_HandleTypeDef *huart, uint8_t addr, uint8_t reg, uint32_t data)
{
    ...
    packet[7] = TMC2209_CalcCRC8(packet, 7U);
...
void BSP_TMC2209_ConfigNode(UART_HandleTypeDef *huart, uint8_t node_addr, uint16_t microsteps, uint8_t irun, uint8_t ihold)
{
    ...
    TMC2209_WriteReg(huart, node_addr, TMC2209_REG_GCONF, gconf);

    uint32_t mres = TMC2209_ConvertMicrostepsToMres(microsteps);
    uint32_t chopconf = TMC2209_CHOPCONF_BASE |
                        TMC2209_CHOPCONF_INTPOL |
                        (mres << TMC2209_CHOPCONF_MRES_SHIFT);
    TMC2209_WriteReg(huart, node_addr, TMC2209_REG_CHOPCONF, chopconf);
    ...
    TMC2209_WriteReg(huart, node_addr, TMC2209_REG_IHOLD_IRUN, ihold_irun);
}
