...
/** 1 Hz encoder status reporting (G-code mode only). / 1 Hz 编码器状态报告(仅 G-code 模式)。 */
...
/** 50 Hz PID closed-loop position hold (G-code mode only). / 50 Hz PID 闭环位置保持(仅 G-code 模式)。 */
...
/** G-code receive + execute pipeline (G-code mode only). / G-code 接收+执行流水线(仅 G-code 模式)。 */
...
int main(void)
{
  ...
  /* ── Control layer initialization / 控制层初始化 ── */
  ...
  /* ── Start motion engine interrupt (TIM6, 50 kHz) / 启动运动引擎中断 (TIM6, 50 kHz) ── */
  extern TIM_HandleTypeDef htim6;
  HAL_TIM_Base_Start_IT(&htim6);

  /* ── Status LED pattern / 状态 LED 模式 ── */
  LedState_t led_pattern[LED_COUNT] = {LED_OFF, LED_OFF, LED_ON, LED_OFF};
  BSP_LED_SetAllStates(led_pattern);
